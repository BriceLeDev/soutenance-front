...
export class AbonnementDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (savedAmount) {
      ...
      this.totalAmount =
        this.totalAmount *
        this.calculateAbnAmount(this.startDate, this.endDate);
    }
  }
  ...
  public haveVideoPan(): boolean {
    return this.selectedPanneauAray.some(
      (selectPan) => selectPan.typePanneauLibele == 'Numérique-vidéo'
    );
  }
  //Vérifie si la liste des panneau contient un panneau Numérique-image
  public haveImagePan(): boolean {
    return this.selectedPanneauAray.some(
      (selectPan) => selectPan.typePanneauLibele == 'Numérique-image'
    );
  }
  ...
  public showAmount(): number {
    return (
      this.totalAmount *
      Math.ceil(this.calculateAbnAmount(this.startDate, this.endDate))
    );
  }

  public clickOnAbnbtn() {
    if (this.startDate == null || this.endDate == null) {
      this.toastr.error('Veuillez définir les dates début et fin.', 'Conflit', {
  ...
  public saveAbonnement() {
    ...
    this.abonnementRequest.Panneau = this.selectedPanneauAray.flatMap((pan) =>
      pan.id != undefined ? [pan.id] : []
    );
    ...
    if (this.isFidel) {
      this.abonnementRequest.prix =
        (this.finalAbnAmount() + this.totalPrintPrice()) * 2;
    } else {
      this.abonnementRequest.prix =
  ...
  private addImage(id: number) {
    ...
    this.imageService.saveImae(requestBody).subscribe({
      next: (resp) => {
        console.log(resp);
      },
      error: (err) => {
  ...
  private addPayement(idAbn: number) {
    ...
    this.transactionRequest.amount =
      this.finalAbnAmount() + this.totalPrintPrice();
        ...
        next: (resp) => {
          ...
          if (resp.data?.payment_url) {
            ...
            this.router.navigate(['/customer/payment'], {
              state: { paymentUrl: resp.data.payment_url } // 🔥 Envoi des données avec `state`
            });
          }
          console.info(resp);
